...
public abstract class WebPagesLaunch {
  ...
  protected void chainResolver(WebClient webClient) {
    ...
    if (!config.getNextChainPageAnchorSelector().isPresent()) {// enhance for no next anchor
      ...
      return;
      // throw new LostConfigureItemException("Lost configuration for next page link selector on
      // each page.");
    }
  ...
  protected WebClient createWebClient() {
    if (config.getProxyHost() == null || !config.getProxyHost().isPresent()) {
      ...
      return new WebClient(config.getBrowserVersion(), config.getProxyHost().get(),
          config.getProxyPort());
    }
  }
