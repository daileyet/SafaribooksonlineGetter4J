...
public interface ResourceAgent {
  ...
  /**
   * the real action to change link reference of the resource which represented by the special
   * parameter
   * 
   * @param element HtmlElement
   */
  ...
  /**
   * the real resource name convert function, it will override the
   * {@link ResourceKeep#resourceNameOf(URL)}
   * 
   * @param url URL
   * @return String
   */
  public default String resolve(URL url) {
    ...
    int end = 0;
    /*
     * end = name.indexOf("?"); if (end != -1 && start < end) name = name.substring(start + 1, end);
     * else name = name.substring(start + 1);
     */
    //
  ...
  /**
   * xxx.svg#abc the real resource name convert function, it will called the
   * {@link HtmlResourceKeeper#getResourceNameOfProundSign()}
   * 
   * @param url URL
   * @return String
   */
  public default String resolveOfPoundSign(URL url) {
    ...
    int start = name.lastIndexOf("/");
    /*
     * int end = 0; end = name.indexOf("?"); if (end != -1 && start < end) name =
     * name.substring(start + 1, end); else name = name.substring(start + 1);
     */
    name = name.substring(start + 1);
    name = name.replaceAll("[^0-9A-Za-z_\\-\\.]+", "_");
    return name;
