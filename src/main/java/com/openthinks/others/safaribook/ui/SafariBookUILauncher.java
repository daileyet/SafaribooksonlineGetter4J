...
final class SafariBookUILauncher {
  ...
  private transient WebPagesConfigure baseConfig = null;
  ...
  void start(WebPagesConfigure baseConfig) {
    if (isRunning == true) {
  ...
  private void doGroupTask(final WebPagesConfigure config) {
  ...
  private void loadConfDirBook(final WebPagesConfigure config, File dir) {
  ...
  private WebPagesConfigure wrapper(DownloadBookTaskInfo bookTaskInfo) {
    WebPagesConfigure cf = (WebPagesConfigure) baseConfig.clone();
  ...
  private void executeSingleTask(final DownloadBookTaskInfo task) {
    WebPagesConfigure cf = wrapper(task);
  ...
  private boolean hasGroupTask(WebPagesConfigure config) {
  ...
  final ActionListener BOOK_ADD = (e) -> {
    ...
    task.setProperty(DownloadBookTaskInfo.PAGELINKOFCATALOGSELECTOR,
        "div.catalog_container a[href*='" + getBookNumberFromUrl(bookUrl) + "']");
    task.setProperty(DownloadBookTaskInfo.DOWNLOADED, "false");
    WorklistPanel UI2 = getUI(WorklistPanel.class);
    DefaultListModel<DownloadBookTaskInfo> listModel =
        (DefaultListModel<DownloadBookTaskInfo>) UI2.listWorklist.getModel();
